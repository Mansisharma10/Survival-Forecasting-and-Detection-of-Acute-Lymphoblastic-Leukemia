...
# Load models
survival_model = joblib.load('random_survival_forest_model.joblib')
all_model = load_model("resnet50.h5")
...
# Streamlit UI
st.title("Acute Lymphoblastic Leukemia (ALL) Diagnosis and Survival Forecasting")
...
if option == "Survival Forecasting":
    ...
    wbc_count = st.sidebar.slider("WBC Count", 0, 50, 20)
    rbc_count = st.sidebar.slider("RBC Count", 0, 10, 5)
    hemoglobin = st.sidebar.slider("Hemoglobin", 0, 20, 12)
    platelet_count = st.sidebar.slider("Platelet Count", 50, 500, 150)
    ...
    if st.button("Predict Survival"):
        ...
        plt.title("Predicted Survival Function (Patient-Friendly)", fontsize=16)
        plt.legend()
        st.pyplot(plt)
